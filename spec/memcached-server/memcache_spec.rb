...
describe MemcachedServer::Memcache do

    before(:each) do 
        @memcache = MemcachedServer::Memcache.new()
        @storage = @memcache.storage
        @item_a = MemcachedServer::Item.new("a", 0, 0, 5, "val_a")
        @item_b = MemcachedServer::Item.new("b", 0, 0, 5, "val_b")
        @item_c = MemcachedServer::Item.new("c", 0, 0, 5, "val_c")
    end

    it "should purge expired keys in storage" do
        expired_item = MemcachedServer::Item.new("key", 0, -1, 3, "val")
        item_to_expire = MemcachedServer::Item.new("kee", 0, 0.1, 3, "val")
    ...
    it "should store some data but only if it wasn't updated since it was last fetched" do
        ...
        expect(reply_stored).to eq MemcachedServer::Reply::STORED
        expect(reply_exists).to eq MemcachedServer::Reply::EXISTS
        expect(reply_not_found).to eq MemcachedServer::Reply::NOT_FOUND        
    end
    ...
    it "should store an item only if it isn't already stored and return a reply" do
        ...
        expect(reply_stored).to eq MemcachedServer::Reply::STORED
        expect(reply_not_stored).to eq MemcachedServer::Reply::NOT_STORED
    end
